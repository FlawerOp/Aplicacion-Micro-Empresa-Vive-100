...
@Component({
  ...
  templateUrl: './descargue.page.html',
  styleUrls: ['./descargue.page.scss'],
})
export class DescarguePage implements OnInit {
  ...
  bloquear(CEDULA) {
    this.FECHA = this.acti.snapshot.paramMap.get("FECHA");
    this.datos2['accion'] = 'bloqueardescargue';
  ...
  redireccionar(CEDULA, FECHA) {
    this.router.navigate(["/descargueproducto", CEDULA, FECHA])
  }
